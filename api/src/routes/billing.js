...
// Lazy Stripe init - server boots fine without STRIPE_SECRET_KEY
let _stripe = null;
function stripe() {
  if (!_stripe) {
...
router.post('/checkout', authRequired, requireStripe, async (req, res) => {
  const { plan } = req.body;
...
router.post('/change-plan', authRequired, requireStripe, async (req, res) => {
  const { plan } = req.body;
  const priceId = config.stripe.prices[plan];
  if (!priceId) return res.status(400).json({ error: 'Invalid plan' });
...
router.get('/info', authRequired, async (req, res) => {
  ...
  if (!u?.stripe_customer_id || !stripe()) {
    return res.json({ plan: u?.plan || 'basic', subscription: null, invoices: [] });
  }
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        await query('UPDATE users SET stripe_subscription_id = $1 WHERE stripe_customer_id = $2', [s.subscription, s.customer]);
        break;
      }
      case 'customer.subscription.updated': {
        ...
        await query('UPDATE users SET plan = $1, updated_at = NOW() WHERE stripe_customer_id = $2', [plan, sub.customer]);
        break;
      }
      case 'customer.subscription.deleted': {
        const sub = event.data.object;
        await query("UPDATE users SET plan = 'basic', stripe_subscription_id = NULL, updated_at = NOW() WHERE stripe_customer_id = $1", [sub.customer]);
        break;
      }
    }
  } catch (err) {
